...
Update of this version: optimize the library for constructing multiple graphs over subsets of vectors. Replace all arrays with std::vectors to better support dynamic insertions. This version removes support for parallel processing.
...
- The original Hnswlib will maintain 65536 mutex locks in each ``hnswlib::HierarchicalNSW`` class, which incurs substantial memory overhead when you are under single-threaded environments and need to construct multiple HNSW graphs. We change the lock number to 1.
